...
def stability_test_delta(initial_data):
    ...
    if (touch_max-np.mean(Touch_data_up)) >2:
        return 2
    ...
    # the min mean force
    if np.mean(Touch_data_up_abs)>8:
...
def stability_test_ttset():
    ...
    for i in range(10):
        ...
        if abs(delta)>50 and pval < pmin_value:
            pmin_value = pval
...
def listener(first_pos=None):
    ...
    #抓取稳定性分析
    while stability==0:
        stability = stability_test_delta(contact_touch)
        if stability ==2:
            print('drop')
            break
    if stability!=2:
        ...
        stability = 0
        touch_max = 0
        listener(touch_max_position)

    stability=0
    # rospy.sleep(0.5)
    for i in range(10):
        stability_cur = stability_test_ttset()
        if stability_cur == 1:
